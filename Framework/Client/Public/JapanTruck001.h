...
class CJapanTruck001 final : public CLandObject
{
public:
	enum CARSTATE { STATE_NONE, STATE_IDLE, STATE_EXIT };
private:
	//생성자와 소멸자이며
	CJapanTruck001(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CJapanTruck001(const CJapanTruck001& rhs);
	virtual ~CJapanTruck001() = default;
...
public:
	static CJapanTruck001*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
