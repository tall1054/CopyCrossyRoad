...
class CMonster_Truck final : public CLandObject
{
public:
	enum CARSTATE { STATE_NONE, STATE_IDLE, STATE_EXIT };

public:
	typedef struct tagMonTruck
	{
		_float3		vAxis;
		_float		fAngle;
		_float4		vPosition;
...
private:
	//생성자와 소멸자이며
	CMonster_Truck(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CMonster_Truck(const CMonster_Truck& rhs);
	virtual ~CMonster_Truck() = default;
...
public:
	static CMonster_Truck*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
