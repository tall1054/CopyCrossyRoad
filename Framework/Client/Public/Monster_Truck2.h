...
class CMonster_Truck2 final : public CLandObject
{
public:
	enum CARSTATE { STATE_NONE, STATE_IDLE, STATE_EXIT };

public:
	typedef struct tagMonTruck2
	{
		_float3		vAxis;
		_float		fAngle;
		_float4		vPosition;
...
private:
	//생성자와 소멸자이며
	CMonster_Truck2(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CMonster_Truck2(const CMonster_Truck2& rhs);
	virtual ~CMonster_Truck2() = default;
...
public:
	static CMonster_Truck2*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
