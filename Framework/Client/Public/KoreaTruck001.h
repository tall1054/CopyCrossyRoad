...
class CKoreaTruck001 final : public CLandObject
{
public:
	enum CARSTATE { STATE_NONE, STATE_IDLE, STATE_EXIT };
private:
	//생성자와 소멸자이며
	CKoreaTruck001(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	CKoreaTruck001(const CKoreaTruck001& rhs);
	virtual ~CKoreaTruck001() = default;
...
public:
	static CKoreaTruck001*	Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
