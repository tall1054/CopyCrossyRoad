...
HRESULT CTreeA::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tTreeAPos, pArg, sizeof(TreeAPosition));

	m_pTransform->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_tTreeAPos.vPosition.x, m_tTreeAPos.vPosition.y, m_tTreeAPos.vPosition.z, m_tTreeAPos.vPosition.w));

	return S_OK;
}

_int CTreeA::Tick(_double TimeDelta)
{
...
_int CTreeA::LateTick(_double TimeDelta)
{
...
HRESULT CTreeA::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_TreeA"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	...
	ColliderDesc.vScale = _float3(3.f, 6.f, 3.f);
...
HRESULT CTreeA::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTreeA * CTreeA::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	...
	{
		MSGBOX("Failed to Created CTreeA");
