...
CRoad::CRoad(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CRoad::NativeConstruct_Prototype()
{
...
HRESULT CRoad::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tRoadPos, pArg, sizeof(RoadPosition));

	m_pTransform->Set_State(CTransform::STATE_POSITION,  XMVectorSet(m_tRoadPos.vPosition.x, m_tRoadPos.vPosition.y, m_tRoadPos.vPosition.z, m_tRoadPos.vPosition.w));

	return S_OK;
}

_int CRoad::Tick(_double TimeDelta)
{
...
_int CRoad::LateTick(_double TimeDelta)
{
...
HRESULT CRoad::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Road"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	...
	ColliderDesc.vScale = _float3(50.f, 0.7f, 2.f);
...
CRoad * CRoad::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CRoad*	pInstance = new CRoad(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CRoad");
