#include "stdafx.h"
#include "../Public/JapanTruck002.h"
#include "GameInstance.h"

CJapanTruck002::CJapanTruck002(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CLandObject(pDevice, pDeviceContext)
{
}

CJapanTruck002::CJapanTruck002(const CJapanTruck002 & rhs)
	: CLandObject(rhs)
{
}

HRESULT CJapanTruck002::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CJapanTruck002 NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CJapanTruck002::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(3.f, XMConvertToRadians(90.f)))))
	{
		MSGBOX("Failed to CJapanTruck002 NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CJapanTruck002 SetUp_Components");
		return E_FAIL;
	}

	_float3		fJapanTruck002Scale = _float3(0.1f, 0.1f, 0.1f);
	m_pTransform->Scaled(XMLoadFloat3(&fJapanTruck002Scale));
...
_int CJapanTruck002::Tick(_double TimeDelta)
{
...
_int CJapanTruck002::LateTick(_double TimeDelta)
{
...
HRESULT CJapanTruck002::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CJapanTruck002 Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CJapanTruck002 SetUp_ConstantTable");
		return E_FAIL;
	}
...
void CJapanTruck002::Clear_Monster_Car()
{
}

HRESULT CJapanTruck002::SetUp_Components()
{
	//렌더러 컴포넌트
	...
	//모델 컴포넌트
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_CJapanTruck002"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Car Prototype_Component_Model_CJapanTruck002");
		return E_FAIL;
	}
...
HRESULT CJapanTruck002::SetUp_ConstantTable()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
CJapanTruck002 * CJapanTruck002::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CJapanTruck002*	pInstance = new CJapanTruck002(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create CJapanTruck002");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CJapanTruck002::Clone(void * pArg)
{
	CJapanTruck002*	pInstance = new CJapanTruck002(*this);
