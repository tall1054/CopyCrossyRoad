#include "stdafx.h"
#include "../Public/Wagon.h"
#include "GameInstance.h"

CWagon::CWagon(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CLandObject(pDevice, pDeviceContext)
{
}

CWagon::CWagon(const CWagon & rhs)
	: CLandObject(rhs)
{
}

HRESULT CWagon::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CWagon NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CWagon::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(3.f, XMConvertToRadians(90.f)))))
	{
		MSGBOX("Failed to CWagon NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CWagon SetUp_Components");
		return E_FAIL;
	}

	_float3		fWagonScale = _float3(0.1f, 0.1f, 0.1f);
	m_pTransform->Scaled(XMLoadFloat3(&fWagonScale));
...
_int CWagon::Tick(_double TimeDelta)
{
...
_int CWagon::LateTick(_double TimeDelta)
{
...
HRESULT CWagon::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CWagon Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CWagon SetUp_ConstantTable");
		return E_FAIL;
	}
...
void CWagon::Clear_Monster_Car()
{
}

HRESULT CWagon::SetUp_Components()
{
	//렌더러 컴포넌트
	...
	//모델 컴포넌트
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_CWagon"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Car Prototype_Component_Model_CWagon");
		return E_FAIL;
	}
...
HRESULT CWagon::SetUp_ConstantTable()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
CWagon * CWagon::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CWagon*	pInstance = new CWagon(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create CWagon");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CWagon::Clone(void * pArg)
{
	CWagon*	pInstance = new CWagon(*this);
