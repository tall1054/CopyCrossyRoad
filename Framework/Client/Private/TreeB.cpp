...
HRESULT CTreeB::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tTreeBPos, pArg, sizeof(TreeBPosition));

	m_pTransform->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_tTreeBPos.vPosition.x, m_tTreeBPos.vPosition.y, m_tTreeBPos.vPosition.z, m_tTreeBPos.vPosition.w));

	return S_OK;
}

_int CTreeB::Tick(_double TimeDelta)
{
...
_int CTreeB::LateTick(_double TimeDelta)
{
...
HRESULT CTreeB::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_TreeB"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CTreeB::SetUp_ConstantTable()
{
...
CTreeB * CTreeB::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	...
	{
		MSGBOX("Failed to Created CTreeB");
