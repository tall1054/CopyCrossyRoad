...
CTrain::CTrain(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CTrain::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tTrainPos, pArg, sizeof(TrainPosition));

	//12.5f, -5.f, 17.5f, 1.f

	m_pTransform->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_tTrainPos.vPosition.x, m_tTrainPos.vPosition.y, m_tTrainPos.vPosition.z, m_tTrainPos.vPosition.w));

	return S_OK;
}

_int CTrain::Tick(_double TimeDelta)
{
...
_int CTrain::LateTick(_double TimeDelta)
{
...
HRESULT CTrain::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Train"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CTrain::SetUp_ConstantTable()
{
...
CTrain * CTrain::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CTrain*	pInstance = new CTrain(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CTrain");
