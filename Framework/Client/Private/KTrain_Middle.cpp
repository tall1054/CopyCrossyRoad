#include "stdafx.h"
#include "..\Public\KTrain_Middle.h"
#include "GameInstance.h"

CKTrain_Middle::CKTrain_Middle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CLandObject(pDevice, pDeviceContext)
{
}

CKTrain_Middle::CKTrain_Middle(const CKTrain_Middle & rhs)
	: CLandObject(rhs)
{
}

HRESULT CKTrain_Middle::NativeConstruct_Prototype()
{
...
HRESULT CKTrain_Middle::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tKMiddlePos, pArg, sizeof(KTrainMiddle));

	m_pTransform->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_tKMiddlePos.vPosition.x, m_tKMiddlePos.vPosition.y, m_tKMiddlePos.vPosition.z, m_tKMiddlePos.vPosition.w));
	m_pTransform->Rotation(XMVectorSet(0.f, 1.f, 0.f, 0.f), XMConvertToRadians(m_tKMiddlePos.fAngle));
	m_pTransform->Scaled(XMLoadFloat3(&m_tKMiddlePos.vScale));

	return S_OK;
}

_int CKTrain_Middle::Tick(_double TimeDelta)
{
...
_int CKTrain_Middle::LateTick(_double TimeDelta)
{
...
void CKTrain_Middle::Clear_Monster_Train()
{
...
HRESULT CKTrain_Middle::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_KTrain_Middle"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Train Prototype_Component_Model_KTrain_Middle");
		return E_FAIL;
	}
...
HRESULT CKTrain_Middle::SetUp_ConstantTable()
{
...
CKTrain_Middle * CKTrain_Middle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CKTrain_Middle*		pInstance = new CKTrain_Middle(pDevice, pDeviceContext);
...
CGameObject * CKTrain_Middle::Clone(void * pArg)
{
	CKTrain_Middle*	pInstance = new CKTrain_Middle(*this);
