...
HRESULT CWater::NativeConstruct(void * pArg)
{
	...
	if (nullptr != pArg)
		memcpy(&m_tWaterPos, pArg, sizeof(WaterPosition));

	m_pTransform->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_tWaterPos.vPosition.x, m_tWaterPos.vPosition.y, m_tWaterPos.vPosition.z, m_tWaterPos.vPosition.w));
...
HRESULT CWater::SetUp_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Water"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
