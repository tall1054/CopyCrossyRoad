#include "stdafx.h"
#include "../Public/KoreaTruck002.h"
#include "GameInstance.h"

CKoreaTruck002::CKoreaTruck002(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CLandObject(pDevice, pDeviceContext)
{
}

CKoreaTruck002::CKoreaTruck002(const CKoreaTruck002 & rhs)
	: CLandObject(rhs)
{
}

HRESULT CKoreaTruck002::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CKoreaTruck002 NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CKoreaTruck002::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(3.f, XMConvertToRadians(90.f)))))
	{
		MSGBOX("Failed to CKoreaTruck002 NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CKoreaTruck002 SetUp_Components");
		return E_FAIL;
	}

	_float3		fKoreaTruck002Scale = _float3(0.1f, 0.1f, 0.1f);
	m_pTransform->Scaled(XMLoadFloat3(&fKoreaTruck002Scale));
...
_int CKoreaTruck002::Tick(_double TimeDelta)
{
...
_int CKoreaTruck002::LateTick(_double TimeDelta)
{
...
HRESULT CKoreaTruck002::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CKoreaTruck002 Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CKoreaTruck002 SetUp_ConstantTable");
		return E_FAIL;
	}
...
void CKoreaTruck002::Clear_Monster_Car()
{
}

HRESULT CKoreaTruck002::SetUp_Components()
{
	//렌더러 컴포넌트
	...
	//모델 컴포넌트
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_CKoreaTruck002"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Car Prototype_Component_Model_CKoreaTruck002");
		return E_FAIL;
	}
...
HRESULT CKoreaTruck002::SetUp_ConstantTable()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
CKoreaTruck002 * CKoreaTruck002::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CKoreaTruck002*	pInstance = new CKoreaTruck002(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create CKoreaTruck002");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CKoreaTruck002::Clone(void * pArg)
{
	CKoreaTruck002*	pInstance = new CKoreaTruck002(*this);
