#include "stdafx.h"
#include "../Public/KoreaTruck001.h"
#include "GameInstance.h"

CKoreaTruck001::CKoreaTruck001(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CLandObject(pDevice, pDeviceContext)
{
}

CKoreaTruck001::CKoreaTruck001(const CKoreaTruck001 & rhs)
	: CLandObject(rhs)
{
}

HRESULT CKoreaTruck001::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CKoreaTruck001 NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CKoreaTruck001::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(3.f, XMConvertToRadians(90.f)))))
	{
		MSGBOX("Failed to CKoreaTruck001 NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CKoreaTruck001 SetUp_Components");
		return E_FAIL;
	}

	_float3		fKoreaTruck001Scale = _float3(0.1f, 0.1f, 0.1f);
	m_pTransform->Scaled(XMLoadFloat3(&fKoreaTruck001Scale));
...
_int CKoreaTruck001::Tick(_double TimeDelta)
{
...
_int CKoreaTruck001::LateTick(_double TimeDelta)
{
...
HRESULT CKoreaTruck001::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CKoreaTruck001 Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CKoreaTruck001 SetUp_ConstantTable");
		return E_FAIL;
	}
...
void CKoreaTruck001::Clear_Monster_Car()
{
}

HRESULT CKoreaTruck001::SetUp_Components()
{
	//렌더러 컴포넌트
	...
	//모델 컴포넌트
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_CKoreaTruck001"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Car Prototype_Component_Model_CKoreaTruck001");
		return E_FAIL;
	}
...
HRESULT CKoreaTruck001::SetUp_ConstantTable()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
CKoreaTruck001 * CKoreaTruck001::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CKoreaTruck001*	pInstance = new CKoreaTruck001(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create CKoreaTruck001");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CKoreaTruck001::Clone(void * pArg)
{
	CKoreaTruck001*	pInstance = new CKoreaTruck001(*this);
