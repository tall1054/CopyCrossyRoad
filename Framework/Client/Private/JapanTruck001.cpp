#include "stdafx.h"
#include "../Public/JapanTruck001.h"
#include "GameInstance.h"

CJapanTruck001::CJapanTruck001(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CLandObject(pDevice, pDeviceContext)
{
}

CJapanTruck001::CJapanTruck001(const CJapanTruck001 & rhs)
	: CLandObject(rhs)
{
}

HRESULT CJapanTruck001::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CJapanTruck001 NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CJapanTruck001::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(3.f, XMConvertToRadians(90.f)))))
	{
		MSGBOX("Failed to CJapanTruck001 NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CJapanTruck001 SetUp_Components");
		return E_FAIL;
	}

	_float3		fJapanTruck001Scale = _float3(0.1f, 0.1f, 0.1f);
	m_pTransform->Scaled(XMLoadFloat3(&fJapanTruck001Scale));
...
_int CJapanTruck001::Tick(_double TimeDelta)
{
...
_int CJapanTruck001::LateTick(_double TimeDelta)
{
...
HRESULT CJapanTruck001::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CJapanTruck001 Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CJapanTruck001 SetUp_ConstantTable");
		return E_FAIL;
	}
...
void CJapanTruck001::Clear_Monster_Car()
{
}

HRESULT CJapanTruck001::SetUp_Components()
{
	//렌더러 컴포넌트
	...
	//모델 컴포넌트
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_CJapanTruck001"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CMonster_Car Prototype_Component_Model_CJapanTruck001");
		return E_FAIL;
	}
...
HRESULT CJapanTruck001::SetUp_ConstantTable()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
CJapanTruck001 * CJapanTruck001::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CJapanTruck001*	pInstance = new CJapanTruck001(pDevice, pDeviceContext);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create CJapanTruck001");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CJapanTruck001::Clone(void * pArg)
{
	CJapanTruck001*	pInstance = new CJapanTruck001(*this);
