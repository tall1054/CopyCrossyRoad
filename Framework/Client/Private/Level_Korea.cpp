...
CLevel_Korea::CLevel_Korea(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
	: CLevel(pDevice, pDeviceContext)
{

}

HRESULT CLevel_Korea::NativeConstruct()
{
...
_int CLevel_Korea::Tick(_double TimeDelta)
{
...
_int CLevel_Korea::LateTick(_double TimeDelta)
{
...
HRESULT CLevel_Korea::Ready_Lights()
{
	CGameInstance*	pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CLevel_Korea::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Camera_Player"), &CameraDesc)))
		return E_FAIL;
...
HRESULT CLevel_Korea::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Terrain"))))
	{
	...
	for (_uint j = 0; j < 4; ++j)
	{
		...
		Grass0 = Grass0 + 2.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Grass"), &GrassPos)))
		{
	...
	for (_uint i = 0; i < 1; ++i)
	{
		...
		Train1 = Train1 + 2.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Train"), &TrainPos)))
		{
	...
	for (_uint i = 0; i < 2; ++i)
	{
		...
		Road1 = Road1 + 2.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Road"), &RoadPos)))
		{
	...
	for (_uint i = 0; i < 2; ++i)
	{
		...
		Water1 = Water1 + 2.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Water"), &WaterPos)))
		{
...
HRESULT CLevel_Korea::Ready_Layer_Player(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);


	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Psy"))))
	{
		MSGBOX("Failed to GamePlay Prototype_GameObject_Psy");
		return E_FAIL;
	}

	/*if (FAILED(pGameInstance->Add_GameObject(Level_Korea, pLayerTag, TEXT("Prototype_GameObject_Samurai"))))
	return E_FAIL;
...
HRESULT CLevel_Korea::Ready_Layer_Monster(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	//=======================Train============================//
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_Train"), &FrontPos)))
	{
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_TrainMiddle"), &MiddlePos)))
	{
	...
	if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_TrainEnd"), &EndPos)))
	{
	...
	for (_uint i = 0; i < 10; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_Car"), &CarPos)))
		{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_KoreaCar001"), &KorCarPos1)))
		{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_KoreaCar002"), &KorCarPos2)))
		{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_KoreaCar003"), &KorCarPos3)))
		{
	...
	for (_uint i = 0; i < 15; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Muccle"), &MucclePos1)))
		{
	...
	//if (FAILED(pGameInstance->Add_GameObject(Level_Korea, pLayerTag, TEXT("Prototype_GameObject_Monster_Taxi"))))
	//{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		Truck1 = Truck1 + 20.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_Truck"), &MonTruck1)))
		{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		Truck2 = Truck2 + 17.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_Truck2"), &MonTruck2)))
		{
	...
	for (_uint i = 0; i < 20; ++i)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Monster_OilTruck"), &OilTruckPos)))
		{
...
HRESULT CLevel_Korea::Ready_Layer_Item(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	for (_uint i = 0; i < 25; ++i)
	{
		...
		Item1 = Item1 + rand() % 10;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Item"), &ItemPos)))
		{
...
HRESULT CLevel_Korea::Ready_Layer_Effect(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	//for (_uint i = 0; i < 30; ++i)
	//{
	//	if (FAILED(pGameInstance->Add_GameObject(Level_Korea, pLayerTag, TEXT("Prototype_GameObject_Effect"))))
	//		return E_FAIL;
...
HRESULT CLevel_Korea::Ready_Layer_Structure(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	for (_uint i = 0; i < 5; ++i)
	{
		...
		TreePos.vPosition = _float4(1.f + Tree1, 0.7f, 1.f, 1.f);
		Tree1 = Tree1 + 1.f;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Tree"), &TreePos)))
		{
			MSGBOX("Failed to Game Play Prototype_GameObject_Tree");
			return E_FAIL;
		}
		TreePos.vPosition = _float4(Tree1, 0.7f, 3.f, 1.f);
		Tree1 = Tree1 + rand() % 15;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Tree"), &TreePos)))
		{
			MSGBOX("Failed to Game Play Prototype_GameObject_Tree");
		...
		TreePos.vPosition = _float4(Tree1, 0.7f, 5.f, 1.f);
		Tree1 = Tree1 + rand() % 5;
		if (FAILED(pGameInstance->Add_GameObject(LEVEL_KOREA, pLayerTag, TEXT("Prototype_GameObject_Tree"), &TreePos)))
		{
			MSGBOX("Failed to Game Play Prototype_GameObject_Tree");
...
HRESULT CLevel_Korea::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CLevel_Korea * CLevel_Korea::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext)
{
	CLevel_Korea*	pInstance = new CLevel_Korea(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct()))
	{
		MSGBOX("Failed to Creating CLevel_Korea");
