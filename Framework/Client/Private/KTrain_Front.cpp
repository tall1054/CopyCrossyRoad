#include "stdafx.h"
#include "..\Public\KTrain_Front.h"
#include "GameInstance.h"

CKTrain_Front::CKTrain_Front(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CLandObject(pDevice, pDeviceContext)
{
}

CKTrain_Front::CKTrain_Front(const CKTrain_Front & rhs)
	: CLandObject(rhs)
{
}

HRESULT CKTrain_Front::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to CKTrain_Front NativeConstruct_Prototype");
		return E_FAIL;
	}
	return S_OK;
}

HRESULT CKTrain_Front::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(&CTransform::TRANSFORMDESC(30.f, XMConvertToRadians(270.0f)))))
	{
		MSGBOX("Failed to CKTrain_Front NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("Failed to CKTrain_Front SetUp_Components");
		return E_FAIL;
	}
...
_int CKTrain_Front::Tick(_double TimeDelta)
{
...
_int CKTrain_Front::LateTick(_double TimeDelta)
{
...
HRESULT CKTrain_Front::Render()
{
	if (nullptr == m_pModelCom)
	{
		MSGBOX("Failed to CKTrain_Front Render_Model");
		return E_FAIL;
	}

	if (FAILED(SetUp_ConstantTable()))
	{
		MSGBOX("Failed to CKTrain_Front SetUp_ConstantTable");
		return E_FAIL;
	}
...
HRESULT CKTrain_Front::SetUp_Components()
{
	/* For.Com_Renderer */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"),
		TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("Failed to CKTrain_Front Prototype_Component_Renderer");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Failed to CKTrain_Front Prototype_Component_Shader_AnimModel");
		return E_FAIL;
	}

	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_KTrain_Front"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSGBOX("Failed to CKTrain_Front Prototype_Component_Model_KTrain_Front");
		return E_FAIL;
	}
...
HRESULT CKTrain_Front::SetUp_ConstantTable()
{
...
CKTrain_Front * CKTrain_Front::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CKTrain_Front*		pInstance = new CKTrain_Front(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CKTrain_Front");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CKTrain_Front::Clone(void * pArg)
{
	CKTrain_Front*	pInstance = new CKTrain_Front(*this);
